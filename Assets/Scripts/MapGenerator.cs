...
public class MapGenerator : MonoBehaviour {
    ...
    private int procentChanceToSpawnLava = 10;
    private int lavaSpreadChance = 50;
    ...
    void Update() {
        if (Input.GetKey(KeyCode.UpArrow) || Input.GetKey(KeyCode.W)) { //Move camera forward
            mainCam.transform.Translate(Vector3.forward, Space.World);
        }

        if (Input.GetKey(KeyCode.DownArrow) || Input.GetKey(KeyCode.S)) { //Move camera back
            mainCam.transform.Translate(Vector3.back, Space.World);
        }

        if (Input.GetKey(KeyCode.LeftArrow) || Input.GetKey(KeyCode.A)) { //Move camera left
            mainCam.transform.Translate(Vector3.left, Space.World);
        }

        if (Input.GetKey(KeyCode.RightArrow) || Input.GetKey(KeyCode.D)) { //Move camera right
